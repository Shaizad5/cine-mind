...
const MOOD_PRESETS = [
  { label: 'Feel-good & uplifting', emoji: 'happy' },
  { label: 'Dark & thrilling', emoji: 'thrilling' },
  { label: 'Romantic & dreamy', emoji: 'romantic' },
  { label: 'Mind-bending & cerebral', emoji: 'cerebral' },
  { label: 'Nostalgic & warm', emoji: 'nostalgic' },
  { label: 'Intense & emotional', emoji: 'emotional' },
  { label: 'Adventurous & epic', emoji: 'adventure' },
  { label: 'Cozy rainy day vibes', emoji: 'cozy' },
];
  ...
  return (
    <div className="pt-16 min-h-screen" data-testid="mood-matcher-page">
        ...
        <div className="relative max-w-3xl mx-auto px-6 lg:px-12 py-20 md:py-28 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="font-serif text-4xl md:text-6xl font-bold tracking-tight mb-4">
              Mood <span className="text-gradient">Matcher</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-xl mx-auto mb-10">
              Describe your mood, vibe, or feeling — our AI will curate the perfect movies from worldwide cinema.
            </p>
          </motion.div>
          ...
          >
            <textarea
              ...
              placeholder="I'm feeling nostalgic, like watching something from the 90s with great music..."
              rows={3}
              className="w-full bg-white/5 border border-white/10 focus:border-secondary focus:ring-1 focus:ring-secondary p-5 pr-14 rounded-2xl text-base outline-none resize-none placeholder:text-muted-foreground/50"
              data-testid="mood-input"
            />
            <button
              type="submit"
              disabled={loading || !mood.trim()}
              className="absolute bottom-4 right-4 w-10 h-10 rounded-full bg-secondary text-white flex items-center justify-center hover:bg-secondary/80 disabled:opacity-40 transition-all duration-300 glow-secondary"
          ...
          <motion.div
            ...
            data-testid="mood-presets"
          >
            {MOOD_PRESETS.map((preset) => (
              <button
                key={preset.label}
                onClick={() => { setMood(preset.label); handleSubmit(preset.label); }}
                disabled={loading}
                ...
                data-testid={`mood-preset-${preset.emoji}`}
              >
                {preset.label}
              </button>
            ))}
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 pb-16">
        {error && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-red-400 mb-8" data-testid="mood-error">
            {error}
          </motion.div>
        )}
        ...
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16" data-testid="mood-loading">
            ...
            <p className="text-muted-foreground">Analyzing your mood and curating movies...</p>
          </motion.div>
        )}
            ...
            >
              <div className="text-center mb-10">
                ...
                <p className="text-muted-foreground">
                  Based on: <span className="italic text-secondary">"{results.mood}"</span>
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {results.recommendations.map((rec, i) => (
                  <motion.div
                    ...
                    className="glass rounded-xl overflow-hidden group hover:border-secondary/20 transition-all duration-300"
                    data-testid={`mood-result-${i}`}
                  >
                    <div className="flex gap-4 p-5">
                      {rec.tmdb?.poster_url ? (
                        ...
                        <div className="w-28 flex-shrink-0 aspect-[2/3] bg-muted rounded-lg flex items-center justify-center">
